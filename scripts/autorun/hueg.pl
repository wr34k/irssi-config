...
%IRSSI = (
  authors     => 'LIFELIKE',
  contact     => 'i@wnt2die.com',
  name        => 'hueg',
  description => 'make text hueg LOL',
...
sub hueg {
  ...
  $in =~ s/\s+$//;
  if ($in eq '') {
    print CRAP "/hueg <string> [options]";
    print CRAP "      -rep <num>     num of times to scroll msg";
    print CRAP "      -re            reverses text";
    print CRAP "      -flip          flips text";
    print CRAP "      -mir           mirrors your text [NOT WORKIN LOL]";
    print CRAP "      -scale <num>   scales shit";      
    print CRAP "      num,num,num    fg, shadow, bg colors (bg optional)";
  }
  else {
    until ($rep==0) {
      colors();
...
sub process {
  for $n (@s1) { #each line
	...
	$whitespace = " " x $scale;
      ...
      $server->command("MSG $witem->{name} $all");
      $all = '';
    }
